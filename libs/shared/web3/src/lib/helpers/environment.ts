...
export class EnvHelper {
   ...
   */
  static getAPIUris() {
    const ALL_URIs = EnvHelper.getSelfHostedNode();
    if (EnvHelper.env["NODE_ENV"] === "development" && ALL_URIs.length === 0) {
      // push in the common ethers key in development
      ALL_URIs.push("https://rpc.ftm.tools");
    }
    if (ALL_URIs.length === 0) ALL_URIs.push("https://rpc.ftm.tools");
    return ALL_URIs;
  }
  ...
  static getGeoapifyAPIKey() {
    const apiKey = EnvHelper.env["REACT_APP_GEOAPIFY_API_KEY"];
    if (!apiKey) {
